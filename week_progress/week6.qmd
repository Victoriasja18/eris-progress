...
def precision_recall(df1, df2, buffer):
    ...
    AND its name (or a synonym) matches. Any prediction whose location
    matches but whose name doesn't is now scored as FP, not silently dropped.
    """
    ...
    for i, truth_idxs in by_pred.items():
        ...
        for j in truth_idxs:
            truth_name = df1.loc[j, "name"]

            name_match = pred["name"] == truth_name
            if not name_match:
                syns = pred.get("synonyms", pd.NA)
